...
class UserController extends Controller {
				...
				if(\Auth::check())
				{
					...
					} else if($row->active=='Banned')
					{
						...
						$session = array(
							...
							'join'	=>  $row->created_at ,
							'level'	=> $level,
							'sso_uuid'	=> $row->sso_uuid,
							'sso_username'	=> $row->sso_username,
							'sso_email'	=> $row->sso_email,
							'sso_last_login'	=> $row->sso_last_login,
							'sso_date_registered'	=> $row->sso_date_registered,
							'sso_date_last_updated'	=> $row->sso_date_last_updated,
							'sso_birthday'	=> $row->sso_birthday,
							'sso_firstname'	=> $row->sso_firstname,
							'sso_lastname'	=> $row->sso_lastname,
							'sso_gender'	=> $row->sso_gender,
							'sso_phone'	=> $row->sso_phone,
							'sso_province'	=> $row->sso_province,
							'sso_city'	=> $row->sso_city,
							'sso_profilepic'	=> $row->sso_profilepic,
							'no_mypoin'	=> $row->no_mypoin,
							'no_ponta'	=> $row->no_ponta
	...
	public function postGojekWebHookHandle(Request $request) {

			if($request->	header("Authorization") == 'Fa3EmQWbGFLtPuZUmEHyFe' && $request->isJson() ){

				\SiteHelpers::auditWebhook($request->input('booking_id') , serialize($request->all()),  $request->input('liveTrackingUrl') );
	...
	public function getSso() {
		...
		if(\Auth::check())
		{
			...
			if (isset($_GET['code'])) {
				...
				$code = $_GET['code'];
				// authorization url
				$url = \Config::get('constants.oauth2_token_url');
				// this will be our POST data to send back to the OAuth server in exchange
				// for an access token
				$params = array(
					"code"          => $code,
					"client_id"     => \Config::get('constants.oauth2_client_id'),
					"client_secret" => \Config::get('constants.oauth2_secret'),
					"redirect_uri"  => \Config::get('constants.oauth2_redirect'),
				...
				$url = \Config::get('constants.oauth2_user_info');
				$client = new \GuzzleHttp\Client();
				$response = $client->get($url,  [  'headers' => [ 'Authorization' => sprintf("Bearer %s", $responseObj->access_token)]]);
				...
				if (!property_exists($responseObj,'error')) {
					...
					## Cek Apakah UUID nya sudah ada ?
					$sso_uuid = $responseObj->uid;

					$users = \DB::table('tb_users')->where('sso_uuid',$sso_uuid)->get();
					## Jika belum buatkan lengkap -> login
					## Jika sudah, langsung login by uuid, by id

					if(count($users)){
						$row = $users[0];
						\Auth::loginUsingId($row->id, true);
						if(\Auth::check()){
							$session = array(
								'gid' => $row->group_id,
								'uid' => $row->id,
								...
								'join'	=>  $row->created_at,
								'sso_uuid'	=> $responseObj->uid,
								'sso_username'	=> $responseObj->username,
								'sso_email'	=> $responseObj->email,
								'sso_last_login'	=> $responseObj->last_login,
								'sso_date_registered'	=> $responseObj->date_registered,
								'sso_date_last_updated'	=> $responseObj->date_last_updated,
								'sso_birthday'	=> $responseObj->birthday,
								'sso_firstname'	=> $responseObj->firstname,
								'sso_lastname'	=> $responseObj->lastname,
								'sso_gender'	=> $responseObj->gender,
								'sso_phone'	=> $responseObj->phone,
								'sso_province'	=> $responseObj->province,
								'sso_city'	=> $responseObj->city,
								'sso_profilepic'	=> $responseObj->profilepic
						...
						$code = rand(10000,10000000);
						$authen = new User;
						$authen->username = $responseObj->username;
						$authen->first_name = $responseObj->firstname;
						$authen->last_name = $responseObj->lastname;
						$authen->email = trim($responseObj->email);
						$authen->activation = $code;
						$authen->group_id = $this->config['cnf_group'];
						$authen->password = \Hash::make("n3sk4f3");
						$authen->active = '1';
						$authen->sso_uuid = $responseObj->uid;
						$authen->sso_username = $responseObj->username;
						$authen->sso_email = trim($responseObj->email);
						$authen->sso_firstname = $responseObj->firstname;
						$authen->sso_lastname = $responseObj->lastname;
						$authen->sso_phone = $responseObj->phone;
						$authen->sso_province = $responseObj->province;
						$authen->sso_city = $responseObj->city;
						$authen->sso_profilepic = $responseObj->profilepic;
						$authen->save();
					...
					//var_dump($responseObj->error); die;
				}

			} else {
				//var_dump("EROR BOSS NOT GET CODE"); die;
