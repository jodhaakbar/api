...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	{
		$data = array(
			'id_trx_booking'	=> $id_trx_booking,
			'request'	=> $request,
			'response_code'		=> $response_code,
			'response'		=> $response
		);

		\DB::table( 'tb_logs_booking')->insert($data);

	}

	public static function auditWebhook( $booking_id, $request, $liveTrackingUrl )
	{
		$data = array(
		...
		\DB::table( 'tb_logs_webhook')->insert($data);
  ...
  static function ResumeUserStatus()
  {
	...
	static public function urlLoginSso(){
		...
		$forward_url =  Config::get('constants.oauth2_auth_url'). '?' . http_build_query($query_params);

		return $forward_url;

	}

	static public function urlProfileSso(){
	...
	static public function urlRegisterSso(){
	...
	static public function urlBase(){
		...
		return "https://kickstart.nescafe.co.id/";
